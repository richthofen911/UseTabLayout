...
public class DatabaseHelper extends OrmLiteSqliteOpenHelper {
    ...
    private static Dao<Ap1Beacon, Integer> beaconDao = null;
    private RuntimeExceptionDao<Ap1Beacon, Integer> beaconRuntimeExceptionDao = null;
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase, ConnectionSource connectionSource){
        try{
            ...
            TableUtils.createTable(connectionSource, Ap1Beacon.class);
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, ConnectionSource connectionSource, int oldVersion, int newVersion) {
        try {
            TableUtils.dropTable(connectionSource, Ap1Beacon.class, true);
    ...
    public Dao<Ap1Beacon, Integer> getBeaconDao() throws SQLException{
        if (beaconDao == null){
            beaconDao = getDao(Ap1Beacon.class);
    ...
    public RuntimeExceptionDao<Ap1Beacon, Integer> getBeaconRuntimeExceptionDao(){
        if (beaconRuntimeExceptionDao == null){
            beaconRuntimeExceptionDao = getRuntimeExceptionDao(Ap1Beacon.class);
    ...
    public void saveBeacon(Ap1Beacon newBeacon){
        if(beaconDao == null){
            try{
                beaconDao = getBeaconDao();
            } catch(SQLException e){
    ...
    public List<Ap1Beacon> queryForAllBeacons(){
        try {
            if(beaconDao == null)
                beaconDao = getBeaconDao();
     ...
     * @return
     */
    public List<Ap1Beacon> queryBeacons(Ap1Beacon beaconFromDetectedList){
        try {
            if(beaconDao == null)
                beaconDao = getBeaconDao();
            List<Ap1Beacon> beaconWanted = beaconDao.queryBuilder().where().
    ...
    public List<Ap1Beacon> queryForBeaconsByCompanyId(String idcompany){
        try {
            List<Ap1Beacon> beaconByTheCompany = beaconDao.queryBuilder().where().
    ...
    public List<Ap1Beacon> queryForBeaconsWithAppIdParent(String idparent){
        try {
            List<Ap1Beacon> beaconWithAppIdParent = beaconDao.queryBuilder().where().
    ...
    public String[] queryDistinctBeaconTable(String columnName){
        try{
            List<Ap1Beacon> distinctSamples = beaconDao.queryBuilder().distinct().selectColumns(columnName).query();
            int resultsAmount = distinctSamples.size();
            String[] results = new String[resultsAmount];
            for(int i = 0; i < resultsAmount; i++){
                results[i] = ((Ap1Beacon)((ArrayList)distinctSamples).get(i)).getIdcompany();
            }
            return results;
        }catch (SQLException e) {
            return null;
